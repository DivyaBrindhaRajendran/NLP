...
def learn():
    ...
    if a1 <= 0 or a2 <= 0:
        for w in words:
            if a1 <= 0:
                weights[w][0] = weights[w][0] + (y1 * words[w])
                cached_weights[w][0] = cached_weights[w][0] + (y1 * c * words[w])
            if a2 <= 0:
                weights[w][1] = weights[w][1] + y2 * words[w]
                cached_weights[w][1] = cached_weights[w][1] + (y2 * c * words[w])
...
"""
all_lines = []
all_lines.append(["i liked the stay", True, False, True, False])
all_lines.append(["i hated the stay very bad", False, True, True, False])
all_lines.append(["i liked the stay. It was extraordinary and amazing", True, False, False, True])
all_lines.append(["i extraordinary the amazing and very", True, False, False, True])
all_lines.append(["i did not liked the stay", False, True, True, False])
"""
