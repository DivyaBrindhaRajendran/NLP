...
def my_l2_reg(net):
    tf.losses.add_loss(
        net,
        loss_collection = tf.GraphKeys.REGULARIZATION
    )
...
def FirstLayer(net, l2_reg_val, is_training):
   ...
   #l2_reg = tf.contrib.layers.l2_regularizer(l2_reg_val)
    ...
    net = tf.contrib.layers.fully_connected(
            net, 40, biases_initializer=None, activation_fn=None)
    x = l2_reg_val*(net**2)
    x = tf.reduce_sum(x)
    tf.losses.add_loss(
        x,
...
# ** TASK 2 ** BONUS part 1
def EmbeddingL2RegularizationUpdate(embedding_variable, net_input, lr, l2_reg_val):
    """Accepts tf.Variable, tensor (batch_size, vocab size), regularization coef.
    Returns tf op that applies one regularization weights on embedding_variable."""
    # TODO(student): Change this to something useful. Currently, this is a no-op.
    net_input = tf.nn.l2_normalize(net_input, 1)
    x = tf.matmul(net_input, embedding_variable)
    x = tf.matmul(tf.transpose(net_input), x)
    r = 2 * lr * l2_reg_val
    y = tf.scalar_mul(r, x)
    embedding_variable -= y
    return embedding_variable


# ** TASK 2 ** BONUS part 2
def EmbeddingL1RegularizationUpdate(embedding_variable, net_input, lr, l1_reg_val):
    """Accepts tf.Variable, tensor (batch_size, vocab size), regularization coef.
    Returns tf op that applies one regularization step on embedding_variable."""
    # TODO(student): Change this to something useful. Currently, this is a no-op.
    net_input = tf.nn.l2_normalize(net_input, 1)
    x = tf.matmul(net_input, embedding_variable)
    x = tf.matmul(tf.transpose(net_input), tf.math.sign(x))
    r = lr * l1_reg_val
    y = tf.scalar_mul(r, x)
    embedding_variable -= y
    return embedding_variable
...
def SparseDropout(slice_x, keep_prob= 0.5):
    ...
    """
    rows, cols = numpy.nonzero(slice_x)
    len_nonzero = round(len(rows)*(1 - keep_prob))
    random_index = numpy.random.choice(len(rows), len_nonzero, replace=False)
    slice_x[rows[random_index], cols[random_index]] = 0
    return slice_x
...
def ComputeTSNE(embedding_matrix):
    ...
    """
    embedding_matrix = TSNE(learning_rate=10, n_iter=400, perplexity=10, n_components= 2).fit_transform(embedding_matrix)
...
def VisualizeTSNE(sess):
    ...
    for label in class_to_words:
        for word in class_to_words[label]:
            index = TERM_INDEX[word]
            x = tsne_embeddings[index][0]
            y = tsne_embeddings[index][1]
            plt.scatter(x, y, color = colors[label])
            plt.text(x + 0.3, y + 0.3, word, fontsize=6)
...
def main(argv):
    ...
    learning_rate = tf.placeholder_with_default(
            numpy.array(0.01, dtype='float32'), shape=[], name='lr')
